  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      if (error.response && error.response.status === 400) {
        setError(error.response.data.msg || "Sign up failed. Please check your details and try again.");
      ...
      setLoading(false); // Set loading to false when sign up completes
    }
  }
    ...
    <div className="login__container container col-sm-12 col-md">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <button className="login__signInButton" type="submit" disabled={loading}>
          {loading ? <Spinner animation="border" size="sm" /> : "Agree and Join"}
        </button>
      </form>
    </div>
  );
